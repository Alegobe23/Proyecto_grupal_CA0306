...
\[
\text{Prima}_{\text{Grupo}} = \frac{VPA\left(\text{Costos Actuariales del grupo}\right)}{VP\left(\text{Masa salarial total del grupo}\right)}
\]
...
- \(S_{\text{corte}}\): salario anual del grupo a la fecha de corte.  
...
\[
VP\left(\text{Masa salarial}\right) = \sum_{k=0}^{T-1} v^k \cdot S_{2025+k}
\]
...
Esta metodología es análoga al cálculo de una anualidad.
...
\[
\text{Prima media nivelada} = \frac{VPA\left(\text{Costos Actuariales totales}\right)}{VP\left(\text{Masa salarial total}\right)}
\]
...
Para el análisis de sensibilidad de las tasas, se calcularon los costos totales nominales (i.e., la suma de los flujos anuales esperados brutos) y el valor presente actuarial de los costos (i.e., la suma de los flujos anuales esperados traídos a valor presente) al reducir y aumentar en un punto porcentual la tasa de incremento salarial (2% anual originalmente) y la tasa de descuento (6,5% anual) para cada beneficio en el plan. El objetivo fue observar la tendencia general, cuantificar las diferencias y ofrecer un razonamiento que justifique los resultados.
...
En diagonal, los razonamientos son los mismos que para el Lump Sum, pero las diferencias son más amplias:  
- De izquierda a derecha, las variaciones son menores en magnitud, aunque proporcionalmente más altas que en el Lump Sum.  
- De derecha a izquierda, las diferencias son más significativas y se alejan más del valor original.  
